    ...
    class TrustCommerceGateway < Base
      ...
      def amount(money)
        ...
        when Integer
          ...
          raise ArgumentError, 'money amount must be either a Money object or a integer in cents.'
        end
      ...
      def add_creditcard(parameters, creditcard)
        ...
        parameters[:address1]  = creditcard.street if creditcard.street?
        parameters[:city]      = creditcard.city if creditcard.city?
        parameters[:state]     = creditcard.state if creditcard.state?
        parameters[:zip]       = creditcard.zip_code if creditcard.zip_code?
