      ...
      module SingletonMethods
        ...
        def find_tagged_with(options = {}) 
          ...
          sql = "SELECT #{o}.* FROM #{jt}, #{o}, #{t} WHERE #{jt}.#{t_fk} = #{t}.#{t_pk} 
                ...
                AND #{o}.#{o_pk} = #{jt}.#{o_fk}"
          sql << " AND #{o}.homebase_id = '#{Homebase.current_homebase.id}'" if options[:homebase]
        ...
        def find_related_tags(tags, options = {})                
          ...
          sql = "SELECT DISTINCT jt.#{o_fk} AS o_id FROM #{jt} jt, #{t} t 
          ...
          sql = "SELECT DISTINCT t.#{t_pk} AS id, t.name AS name, COUNT(jt.#{o_fk}) AS count FROM #{jt} jt, #{t} t 
                 WHERE jt.#{o_fk} IN (#{o_ids.join(",")}) 
                 AND t.#{t_pk} = jt.#{t_fk}
                 GROUP BY t.#{t_pk}, t.name
                 ORDER BY count DESC"
          add_limit!(sql, options)
        ...
        # Finds other tags in the current homebase
        # Pretty simple
        def find_tags_for_homebase
          o, o_pk, o_fk, t, t_pk, t_fk, jt = set_locals_for_sql
          sql = "SELECT DISTINCT #{t}.* FROM #{t}, #{o}, #{jt} 
                 WHERE (#{t}.id = #{jt}.#{t_fk})
                 AND (#{jt}.#{o_fk} = #{o}.#{o_pk})
                 AND (#{o}.homebase_id = #{Homebase.current_homebase.id})"
