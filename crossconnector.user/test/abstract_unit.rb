...
#require 'action_view'
#require File.dirname(__FILE__) + '/file_column_connection'
#require 'stringio'
...
class Test::Unit::TestCase
  ...
  def clear_validations
    [:validate, :validate_on_create, :validate_on_update].each do |attr|
        Resource.write_inheritable_attribute attr, []
        #Movie.write_inheritable_attribute attr, []
      end
  end

  def file_path(filename)
    File.expand_path("#{File.dirname(__FILE__)}/fixtures/resources/#{filename}")
  end
